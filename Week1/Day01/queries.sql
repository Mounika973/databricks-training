...
SELECT YEAR(hire_date) AS hire_year,COUNT(*) AS total_employees FROM Employee GROUP BY YEAR(hire_date);
...
SELECT YEAR(hire_date) AS hire_year,COUNT(*) AS total_hired FROM Employee GROUP BY YEAR(hire_date) HAVING COUNT(*)>1;
...
SELECT * FROM Employee WHERE department_id NOT IN (SELECT department_id FROM Project);

-- 42. Select employees and the number of projects their department is working on.
SELECT e.name,COUNT(p.project_id) AS total_projects FROM Employee e JOIN Project p ON e.department_id=p.department_id GROUP BY e.name;
